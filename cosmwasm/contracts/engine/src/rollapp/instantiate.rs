...
pub fn instantiate(storage: &mut dyn Storage, msg: GenesisState) {
    for elem in msg.rollapp_list.into_iter() {
        state::set_rollapp(storage, elem);
    }

    for elem in msg.state_info_list.into_iter() {
        state::set_state_info(storage, elem);
    }

    for elem in msg.latest_state_info_index_list.into_iter() {
        state::set_latest_state_info_index(storage, elem)
    }

    for elem in msg.latest_finalized_state_index_list.into_iter() {
        state::set_latest_finalized_state_index(storage, elem)
    }

    for elem in msg.block_height_to_finalization_queue_list.into_iter() {
        state::set_block_height_to_finalization_queue(storage, elem)
    }

    state::set_params(storage, msg.params.unwrap())
}
...
mod tests {
    ...
    fn test_instantiate() {
        ...
        let params_res = state::get_params(deps.as_mut().storage).unwrap();
        assert_eq!(params, params_res);

        let rollapp_list = state::get_all_rollapp(deps.as_mut().storage).unwrap();
        assert_eq!(1, rollapp_list.len());
        assert_eq!(rollapp, rollapp_list[0].1);

        let state_info_list = state::get_all_state_info(deps.as_mut().storage).unwrap();
        assert_eq!(1, state_info_list.len());
        assert_eq!(state_info, state_info_list[0].1);

        let latest_state_info_index_list =
            state::get_all_latest_state_info_index(deps.as_mut().storage).unwrap();
        ...
        let latest_finalized_state_index_list =
            state::get_all_latest_finalized_state_index(deps.as_mut().storage).unwrap();
        ...
        let block_height_to_finalization_queue_list =
            state::get_all_block_height_to_finalization_queue(deps.as_mut().storage).unwrap();
